...
const REPORT_TYPES = [
    { id: 'lluvia_debil', label: 'Lluvia Débil', labelEn: 'Light Rain', icon: CloudRain, color: 'bg-blue-500/20 text-blue-400 border-blue-500/50' },
    { id: 'lluvia_fuerte', label: 'Lluvia Fuerte', labelEn: 'Heavy Rain', icon: CloudLightning, color: 'bg-indigo-600/30 text-indigo-300 border-indigo-500/50' },
    { id: 'granizo_pequeno', label: 'Granizo Pequeño', labelEn: 'Small Hail', icon: CloudHail, color: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/50' },
    { id: 'granizo_grande', label: 'Granizo Dañino', labelEn: 'Damaging Hail', icon: AlertTriangle, color: 'bg-red-600/30 text-red-400 border-red-500/50' },
    { id: 'viento_fuerte', label: 'Viento Fuerte', labelEn: 'Strong Wind', icon: Wind, color: 'bg-slate-500/20 text-slate-300 border-slate-500/50' },
    { id: 'cielo_despejado', label: 'Cielo Despejado', labelEn: 'Clear Sky', icon: Sun, color: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/50' },
]
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (existingReport) {
                ...
                alert(t("Reporte actualizado con éxito.", "Report updated successfully."));
            } else {
                // CREATE MODE
                ...
                alert(t("Reporte enviado con éxito. ¡Gracias por colaborar!", "Report sent successfully. Thanks for collaborating!"));
            }
            ...
            alert(t("Error al procesar el reporte. Inténtalo de nuevo.", "Error processing report. Please try again."));
        } finally {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md bg-zinc-950 border-zinc-800 text-zinc-100">
                <DialogHeader>
                    <DialogTitle>{existingReport ? t("Editar Reporte", "Edit Report") : t("Reportar Clima", "Report Weather")}</DialogTitle>
                    <DialogDescription>
                        {existingReport ? t("Modifica la descripción o la foto.", "Modify the description or photo.") : t("¿Qué está pasando en tu ubicación ahora?", "What is happening at your location right now?")}
                    </DialogDescription>
                </DialogHeader>
                ...
                `}
                            >
                                ...
                                <span className="text-xs font-bold">{t(type.label, type.labelEn)}</span>
                            </button>
                        )
                    })}
                </div>
                ...
                <Textarea
                    placeholder={t("Comentario opcional (ej. tamaño del granizo)", "Optional comment (e.g. hail size)")}
                    value={description}
                    ...
                    {!previewUrl ? (
                        <div className="flex items-center gap-3">
                            ...
                            >
                                ...
                                <span>{existingReport ? t("Cambiar Foto", "Change Photo") : t("Adjuntar Foto", "Attach Photo")}</span>
                            </label>
                            <span className="text-xs text-zinc-500 italic">{t("Opcional", "Optional")}</span>
                        </div>
                    ) : (
                ...
                <DialogFooter className="sm:justify-between gap-2">
                    <DialogClose asChild>
                        <Button type="button" variant="ghost" className="text-zinc-400 hover:text-white">{t("Cancelar", "Cancel")}</Button>
                    </DialogClose>
                    <Button
                        type="button"
                    ...
                    >
                        {isSubmitting ? t("Enviando...", "Sending...") : (existingReport ? t("Guardar Cambios", "Save Changes") : t("Enviar Reporte", "Send Report"))}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
